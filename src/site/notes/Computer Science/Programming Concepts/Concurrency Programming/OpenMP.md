...
החישוב המקבילי יהיה דומה למה שעשינו בדוגמאות הקודמות, כל thread יבצע סכום חלקי של שטחי המלבנים ולבסוף בקטע קוד סדרתי יסכום את כל הסכומים החקיים. את הסכומים ניתן לשמור באמצעות מערך שגודלו כמספר הthreads וככה כל thread מקבל משבצת אחת במערך מבלי לדאוג לrace condition (כפי שהראנו, זה יכול לפגוע בביצועים אם לא לוקחים בחשבון את המטמון).
...
```
2) אם רוצים לבצע לולאה פעם אחת בלבד, אבל לחלק אותה לחלקים כך שכל חלק יתבצע על ידי thread אחר אז ניתן להשתמש בpragma omp for בתוך parallel block או pragma omp parallel for. 
...
נשים לב שבדוגמת הקוד האחרון נקבל 2 threads ולא 4 כמו ברוב המקרים הדיפולטים בגלל שהקומפיילר + מערכת ההפעלה החליטו להקצות פחות ממה שהוא יכול כי מספר האיטרציות הוא 2.
...
ניתן לבצע reduce גם על מערכים ולייצר שכפות שלהם לכל thread. 
## Schedule 
...
כאמור משתנים המוגדרים מחוץ לקבע המקבילי הם shared.
משתנים המוגדרים בתוך קבע מקבילי הם private, כלומר כל thread מקבל עותר של משתנים אלו. 
...
i הופך אוטומטי למשתנה פרטי ו j הפל לפרטי בגלל שהשתמשנו בהגדרה private(j). 
